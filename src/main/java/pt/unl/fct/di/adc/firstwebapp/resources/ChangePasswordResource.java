 ...
 * Op9: {@code POST /rest/changeuserpassword} — authenticated caller; {@code input.username} is the account
...
public class ChangePasswordResource {
	...
	public Response changePassword(ChangePasswordRequest body) {
		...
		String callerUserId = session.getString(SessionStore.PROP_USER_ID);
		String targetId = body.input.username.trim();

		if (!callerUserId.equals(targetId)) {
		...
		try {
			...
			return Response.ok(GSON.toJson(new ApiJsonResponse.ChangePasswordSuccess())).build();
