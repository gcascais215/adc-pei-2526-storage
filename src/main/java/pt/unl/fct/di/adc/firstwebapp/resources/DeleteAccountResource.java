...
/**
 * Op4: {@code POST /rest/deleteaccount} — only {@code ADMIN} .
 */
@Path("/deleteaccount")
...
public class DeleteAccountResource {
	...
	private static final String USER_LOG_KIND = "UserLog";
	private static final String USER_STATS_KIND = "UserStats";
	...
	public Response deleteAccount(DeleteAccountRequest body) {
		...
		try {
			...
			SessionStore.deleteAllSessionsForUser(datastore, targetId);

			Query<Entity> logQuery = Query.newEntityQueryBuilder()
					.setKind(USER_LOG_KIND)
					.setFilter(PropertyFilter.hasAncestor(userKey))
					.build();
			QueryResults<Entity> logs = datastore.run(logQuery);
			while (logs.hasNext()) {
				datastore.delete(logs.next().getKey());
			}

			Key statsKey = datastore.newKeyFactory()
					.addAncestors(PathElement.of(USER_KIND, targetId))
					.setKind(USER_STATS_KIND)
					.newKey("counters");
			datastore.delete(statsKey);

			datastore.delete(userKey);

			LOG.info("Account deleted: " + targetId);
			return Response.ok(GSON.toJson(new ApiJsonResponse.DeleteAccountSuccess())).build();
		} catch (Exception e) {
			LOG.log(Level.SEVERE, "deleteAccount failed", e);
